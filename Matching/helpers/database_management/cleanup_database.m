function [ dbt ] = cleanup_database(dbt, remove_rejected, remove_QuarterArbitrary, remove_nan, properties, post_automated,only_erik, legacy_table )
...
if only_erik
    dbt = dbt(dbt.RegistrationType == 'Erik''s Program',:);
end
...
manual_changes = true;
if legacy_table && manual_changes
...
dbt(un_reject_bool,:).SessionRejected = categorical(zeros(sum(un_reject_bool),1) , [0, 1],categories(dbt.SessionRejected));
